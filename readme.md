...
- **Konzistentní správa**: Sjednocené rozhraní pro správu všech sekcí s intuitivním filtrováním podle tagů a rychlým fulltextovým vyhledáváním. Pro maximální přehlednost výběr štítku ve filtrech automaticky vynuluje aktivní vyhledávání.
...
- **Integrace AI (Gemini & OpenAI)**: DevBase využívá sílu umělé inteligence od Google i OpenAI pro usnadnění vaší práce. Funkce jsou dostupné po zadání API klíče v Nastavení:
  ...
  - **Konfigurace**: V nastavení si můžete zvolit **preferovaného poskytovatele (Gemini / OpenAI)** a konkrétní model (např. GPT-4o Mini, GPT-5.2 nebo Gemini 2.5 Flash Lite / Pro / 3.1) a otestovat platnost klíčů. Celou AI integraci lze v nastavení globálně vypnout jedním přepínačem. Všechny AI prvky jsou v UI barevně odlišeny (fialová s premium glow efektem).
