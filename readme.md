...
- **Sekce TODO (Úkoly)**: Nástroj pro správu priorit s podporou vlastních štítků, filtrování, odškrtnutí (okamžitá archivace), trvalého odstranění a řazení pomocí Drag & Drop. Panel pro přidávání úkolů je moderně integrován v designu vyhledávacího baru. Pro urychlení práce se při přidávání nového úkolu automaticky předvybírá první dostupný štítek. Kliknutím na text úkolu se nově otevře **přehledný detailní náhled** (s termínem, poznámkou i štítky), zatímco pro editaci slouží vyhrazené tlačítko. Sekce plně podporuje **Termíny splnění (Deadlines)**, které vás vizuálně upozorní přímo u úkolu. Pro maximální přehlednost je v hlavičce aplikace integrován **systém upozornění (zvoneček)**, který vás barevným odznakem a pulzováním upozorní na blížící se nebo zmeškané termíny, i když zrovna nejste v sekci TODO (přechod z notifikace navíc cílový úkol **vizuálně zvýrazní**). Pomocí funkce **AI Bojový plán** si můžete nechat analyzovat své úkoly a získat od AI strategické doporučení, na co se zaměřit nejdříve.
...
- **Plně AJAXové rozhraní**: Většina akcí v aplikaci nyní probíhá bez obnovování stránky. **Nastavení** se ukládají okamžitě při změně přepínače, tagy a jazyky lze spravovat bleskově a přepínání tabů v draftech je plynulé. Tato modernizace zajišťuje rychlou odezvu a špičkový uživatelský zážitek.
...
- **Propracované UI**: Elegantní rozhraní postavené na Bootstrap 5, oživené plovoucími barevnými prvky, blur efekty, micro-animacemi a moderní vizí glassmorphismu. Významné akce jsou doprovázeny **fialovým flash efektem** pro jasnou vizuální odezvu.
...
- **Integrace AI (Gemini & OpenAI)**: DevBase využívá sílu umělé inteligence od Google i OpenAI pro usnadnění vaší práce. Funkce jsou dostupné po zadání API klíče v Nastavení:
  ...
  - **Konfigurace**: V nastavení si můžete zvolit **preferovaného poskytovatele (Gemini / OpenAI)** a konkrétní model (např. GPT-4o nebo Gemini 2.5 Flash Lite) a otestovat platnost klíčů. Celou AI integraci lze v nastavení globálně vypnout jedním přepínačem, který skryje všechna AI tlačítka v celé aplikaci. Všechny AI prvky jsou v UI barevně odlišeny (fialová s premium glow efektem).
