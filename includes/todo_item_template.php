...
// Subtask progress
$subtaskTotal = count($todo['subtasks'] ?? []);
$subtaskDone = 0; // In this app, "done" means archived, but subtasks in current view are active. 
// However, we might want to show total vs archived in the future.
?>
<div class="todo-wrapper" id="todo-wrapper-<?php echo $todo['id']; ?>">
    <div class="card glass-card todo-item <?php echo $todo['is_pinned'] ? 'pinned' : ''; ?> <?php echo $todo['is_locked'] ? 'locked' : ''; ?> <?php echo $deadlineStatus; ?> <?php echo !empty($todo['parent_id']) ? 'subtask-item' : ''; ?>" 
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center overflow-hidden flex-grow-1">
                    ...
                    <div class="d-flex flex-column overflow-hidden flex-grow-1" style="cursor: pointer;" onclick="openViewTodoModal(<?php echo htmlspecialchars(json_encode($todo), ENT_QUOTES, 'UTF-8'); ?>)">
                        ...
                        <span class="fs-5 text-truncate text-white">
                            ...
                            <?php endif; ?>
                            <span class="badge bg-light bg-opacity-10 text-white-50 ms-2 fw-normal subtask-count-badge <?php echo ($subtaskTotal > 0) ? '' : 'd-none'; ?>" style="font-size: 0.65em;">
                                <i class="bi bi-diagram-2 me-1"></i>
                                <span class="count"><?php echo $subtaskTotal; ?></span>
                            </span>
                        </span>
                ...
                <div class="action-btns flex-shrink-0 ms-3">
                    ...
                    <div class="d-none d-md-flex gap-2">
                        <?php if (empty($todo['parent_id'])): ?>
                        <button type="button" class="btn btn-sm btn-link text-white-50 p-0" 
                                onclick="addSubtask(<?php echo $todo['id']; ?>, event)"
                    ...
                    <div class="dropdown d-md-none">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end dropdown-menu-dark shadow-lg border border-light border-opacity-10">
                            <?php if (empty($todo['parent_id'])): ?>
                            <li>
                                <a class="dropdown-item small py-2" href="javascript:void(0)" onclick="addSubtask(<?php echo $todo['id']; ?>, event)">
                                    <i class="bi bi-plus-circle me-2"></i> Podúkol
                                </a>
                            </li>
                            <?php endif; ?>
                            <?php if (empty($todo['parent_id'])): ?>
    ...
    <?php if (!empty($todo['subtasks'])): ?>
        <div class="subtasks-container ms-4 ms-md-5 mt-2 d-flex flex-column gap-2 border-start border-light border-opacity-10 ps-3">
            <?php 
            $parentTodo = $todo;
            foreach ($parentTodo['subtasks'] as $subtask): 
                $todo = $subtask;
                include __DIR__ . '/todo_item_template.php'; 
            endforeach; 
            $todo = $parentTodo;
            ?>
        </div>
